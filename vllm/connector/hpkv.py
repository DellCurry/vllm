...
def _parse_metadata(d_metadata_buffer: torch.Tensor,
...
class HPKVConnector(BaseKVConnector):
    ...
    def __init__(self, url: str, device: torch.device = "cpu"):
        ...
        self.device = device
        self.connection = hpkv.HPKVTensorClient(parsed_url.hostname,
                                                parsed_url.port, None, 0)
    ...
    def get(self, key: str) -> Optional[torch.Tensor]:
        ...
        if ret != 0:
            logger.error("Failed to get metadata for key %s, ret = %d",
                         meta_key, ret)
            return None
    ...
    def set(self, key: str, tensor: torch.Tensor) -> None:
        ...
        if ret != 0:
            raise ValueError(
                f"Failed to set metadata for key {meta_key}, ret = {ret}")

        ret = self.connection.set(key, tensor)
        if ret != 0:
            raise ValueError(
                f"Failed to set tensor for key {key}, ret = {ret}")
